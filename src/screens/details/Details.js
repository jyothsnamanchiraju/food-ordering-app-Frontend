...
class Details extends Component {
        ...
        if (json.code === undefined) {
          ...
          ReactDOM.render(
            <div className="backend-error">
              ...
              <p>
                <strong>Error code:</strong> {json.code}
              </p>
              <p>
                <strong>Error message:</strong> {json.message}
              </p>
            </div>,
    ...
    if (this.state.cartItems.length === 0) {
      ...
      if (sessionStorage.getItem("access-token") === null) {
        ...
        sessionStorage.setItem(
          "restaurantDetails",
          JSON.stringify(this.state.restaurantData)
        );
        this.props.history.push({
          pathname: "/checkout",
          state: {
            checkoutCartItems: this.state.cartItems,
  ...
  cartDecrementQuantityHandler = (element, message) => {
    ...
    this.setState({
      ...
      totalCost: this.state.totalCost - element.item.price,
    });
  };
        ...
        {this.state.load === true ? (
          <div className="container">
            ...
            <Grid
              container
              spacing={3}
              id="r-details-wrapper-id"
              className="r-details-wrapper"
            >
              {/* Restaurant Image */}
              <Grid item xs={12} md={3} className="r-image">
                <img
                  src={this.state.restaurantData.photo_URL}
                  alt={this.state.restaurantData.restaurant_name}
                />
              </Grid>
              <Grid container item xs={12} md={9} className="r-container-right">
                ...
                <Grid
                  ...
                  xs={12}
                  alignItems="center"
                  className="r-details-container-bottom"
                >
                  <Grid item xs={6} className="r-rating">
                    ...
                    <p className="r-no-of-customers">
                      AVERAGE RATING BY{" "}
                      <strong>{data.number_customers_rated}</strong> CUSTOMERS
                    </p>
                  </Grid>
              ...
              {/* End of class="r-container-right" */}
            </Grid>{" "}
            {/* End of class="r-details-wrapper" */}
              ...
              <div className="category-container">
                {
                  ...
                  data.categories.map((element, i) => (
                    <div key={"cat" + i + 1}>
                      <p key={"catName" + i + 1} className="category-name">
                        {element.category_name}
                      </p>
                      ...
                      {element.item_list.map((item, j) => (
                        ...
                        >
                          <Grid item xs={1} md={1} key={"itemType" + j + 1}>
                            {item.item_type === "VEG" ? (
                              <i
                                className="fa fa-circle veg"
                                aria-hidden="true"
                              ></i>
                            ) : (
                              <i
                                className="fa fa-circle non-veg"
                                aria-hidden="true"
                              ></i>
                            )}
                          </Grid>
                          ...
                          <Grid item xs={1} md={1} key={"plus" + j + 1}>
                            <IconButton
                              className="addIcon"
                              onClick={() =>
                                this.snackbarOpenHandler(
                                  { item },
                                  "Item added to cart!"
                                )
                              }
                          ...
                          >
                            <Grid item xs={1} md={1} key={"itemType" + j + 1}>
                              {cartItem.item.item_type === "VEG" ? (
                                <i
                                  className="fa fa-stop-circle-o veg"
                                  aria-hidden="true"
                                ></i>
                              ) : (
                                <i
                                  className="fa fa-stop-circle-o non-veg"
                                  aria-hidden="true"
                                ></i>
                              )}
                            </Grid>
                            ...
                            >
                              <i
                                className="fa fa-minus cart-minus"
                                aria-hidden="true"
                                onClick={() =>
                                  this.cartDecrementQuantityHandler(
                                    cartItem,
                              ...
                              <i
                                className="fa fa-plus cart-plus"
                                aria-hidden="true"
                                onClick={() =>
                                  this.snackbarOpenHandler(
                                    cartItem,
                            ...
                            >
                              ...
                              <span>
                                {" "}
                                {(
                                  cartItem.item.quantity * cartItem.item.price
                                ).toFixed(2)}
                              </span>
                            </Grid>
                          </Grid>
