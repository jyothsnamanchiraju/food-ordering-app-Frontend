...
class Checkout extends Component {
  ...
  componentWillMount() {
    ...
    this.setState({
      totalCartItemsValue: this.props.history.location.state.totalAmount,
    });
    this.setState({
  ...
  addressClickHandler = () => {
    ...
    xhrSaveAddress.addEventListener("readystatechange", function () {
      if (this.readyState === 4) {
        ...
        if (
          saveAddressResponse.code === "SAR-002" ||
          saveAddressResponse.code === "SAR-002"
        ) {
...
*/
  ...
  checkoutHandler = () => {
    if (
      sessionStorage.getItem("selAddress") === "null" ||
      sessionStorage.getItem("selAddress") === null
    ) {
        ...
        <Grid container spacing={1}>
          ...
          <Grid item xs={8} md={3}>
            <Card>
              ...
              >
                {this.state.resDetails.restaurant_name}
              </div>
              <CardContent>
                <Grid
                  container
                  ...
                  alignItems="center"
                >
                  {this.props.history.location.state.checkoutCartItems.map(
                    (item, index) => {
                      return (
                        <Grid
                          style={{
