  ...
  componentWillMount() {
    ...
    xhr.addEventListener("readystatechange", function () {
      if (this.readyState === 4) {
        that.setState({
          listOfAllRestaurants: JSON.parse(this.responseText).restaurants,
        });
      }
    });
  ...
  searchRestaurantByName = (resName) => {
    ...
    if (resName !== "" && resName !== " ") {
      ...
      xhr.addEventListener("readystatechange", function () {
        if (this.readyState === 4) {
          that.setState({
            listOfRestaurantsBySearch: JSON.parse(this.responseText)
              .restaurants,
          });
        }
      });
    ...
    return (
      <div>
        <div className="home-hdr">
          <Header
            searchRestaurantByName={this.searchRestaurantByName}
              ...
              >
                {restaurants.map((res) => (
                  <GridListTile key={res.id}>
                    <Card
                      className="card-root"
                      onClick={() => this.restaurantClickHandler(res.id)}
                    >
                      <img
                        id={res.id}
                        src={res.photo_URL}
